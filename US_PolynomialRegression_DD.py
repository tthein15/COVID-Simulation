...
plt.title('Daily Deaths In US: Polynomial Regression Fits')
...
# DataFrame
PRfits_US_DD = pd.DataFrame({'Dates': c, 'Cases':y1test, 'Degree = 4':y_poly_pred2,'Degree = 6':y_poly_pred4})
PR_November_DD = PRfits_US_DD.to_excel("USPolynomialRegressionNovemberDeaths.xlsx", sheet_name='NovemberDailyDeathsPrediction')


# Evaluate 5th and 6th Degree Fit(Best fit)
coef2 = model3.coef_
intercept3 = model2.intercept_

coef4 = model4.coef_
intercept4 = model4.intercept_

PR5BestEval_US_DD = pd.DataFrame({'R2':[r2_d4],'RMSE':[rmse4],'RMSLE':[rmsle4]})    
PR6BestEval_US_DD = pd.DataFrame({'R2':[r2_d6],'RMSE':[rmse6],'RMSLE':[rmsle6]})
...
plt.figure(figsize=(12,5)) 
plt.grid()
plt.scatter(np.arange(-6,1,1),y[297:304], facecolor = 'none', edgecolor = 'blue')
...
plt.title('Daily Deaths In US: Polynomial Regression(Predictions)')
plt.ylabel('Daily Deaths')
...
PR_Pred_US_DD = pd.DataFrame({'Days since 11/23':future_days_gr,'Degree = 4':future_pred_2,'Degree = 6':future_pred_4})
PR_December_DD = PR_Pred_US_DD.to_excel("USPolynomialRegressionDecemberDeaths.xlsx", sheet_name='DecemberDailyDeathsPrediction')
