...
ytest = ytest.tolist()
SVRfit_Brazil_DC = pd.DataFrame({'Dates': c, 'Cases Prediction':y_pred, 'Cases':ytest})
SVR_November_DC = SVRfit_Brazil_DC.to_excel("BrazilSupportVectorRegressionNovemberCases.xlsx", sheet_name='November Daily Cases Prediction')
...
plt.title('Daily Cases In Brazil: SVR')
...
# Excel File    
SVR_BrazilDC_pred = pd.DataFrame({'Days since 11/23':future_days , 'Cases Prediction':future_pred})
SVR_December_DC = SVR_BrazilDC_pred.to_excel("BrazilSupportVectorRegressionDecemberCases.xlsx", sheet_name='December Daily Cases Prediction')

print('------------SVR EVALUATION-------------')
print('RMSE Score: '+ np.str(rmse))
print('RMSLE Score: '+ np.str(rmsle))
print('R2 Score: '+ np.str(R2))
print(SVRfit_Brazil_DC)
print(SVR_BrazilDC_pred)
