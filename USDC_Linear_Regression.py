...
plt.plot(c, y_pred, 'r--')
plt.title('US Daily Cases: Linear Regression')
plt.xlabel('Days')
plt.ylabel('Cases')
#plt.show()
...
LRfit_US_DC = pd.DataFrame({'Dates': c, 'Cases Prediction':y_pred, 'Cases':y1test})
LR_US_DC = LRfit_US_DC.to_excel("USLinearRegressionNovemberCases.xlsx", sheet_name='NovemberDailyCasesPrediction')
...
LR_USDC_pred = pd.DataFrame({'Days since 11/23':future_days , 'Cases Prediction':future_pred})
LR_US_DC = LR_USDC_pred.to_excel("USLinearRegressionDecemberCases.xlsx", sheet_name='DecemberDailyCasesPrediction')

# Evaluation Table
LR_US_Evaluation_DC = pd.DataFrame({'Model':lin_eq,'RMSE':[rmse], 'RMSLE':[rmsle], 'R2': [R2]})
