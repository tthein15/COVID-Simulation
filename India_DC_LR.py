...
plt.plot(c, y_pred, 'r--')
plt.title('India Daily Cases: Linear Regression')
plt.xlabel('Months')
plt.ylabel('Cases')
#plt.show()
...
LRfit_India_DC = pd.DataFrame({'Dates': c, 'Cases Prediction':y_pred, 'Cases':y1test})
LR_India_DC = LRfit_India_DC.to_excel("IndiaLinearRegressionNovemberCases.xlsx", sheet_name='NovemberDailyCasesPrediction')
...
LR_IndiaDC_pred = pd.DataFrame({'Days since 11/23':future_days , 'Cases Prediction':future_pred})
LR_India_DC = LR_IndiaDC_pred.to_excel("IndiaLinearRegressionDecemberCases.xlsx", sheet_name='DecemberDailyCasesPrediction')

# Evaluation Table
LR_India_Evaluation_DC = pd.DataFrame({'Model':lin_eq,'RMSE':[rmse], 'RMSLE':[rmsle], 'R2': [R2]})
...
print(LRfit_India_DC)
print(LR_IndiaDC_pred)
