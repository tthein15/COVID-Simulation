...
y_poly_pred4 = model4.predict(x_poly_test4)
...
# DataFrame
PRfits_Brazil_DC = pd.DataFrame({'Dates': c, 'Deaths':y1test, 'Degree = 4':y_poly_pred2,'Degree = 5':y_poly_pred3})
...
# Evaluate Quadratic and 5th Degree Fit(Best fit)
coef2 = model2.coef_
...
PR5BestEval_Brazil_DC = pd.DataFrame({'R2':[r2_d5],'RMSE':[rmse5],'RMSLE':[rmsle5]})
...
plt.figure(figsize=(12,5)) 
plt.grid()
plt.scatter(np.arange(-6,1,1),y[266:273], facecolor = 'none', edgecolor = 'blue')
...
plt.title('Daily Cases In Brazil: Polynomial Regression(Predictions)')
plt.ylabel('Daily Cases')
...
PR_Pred_Brazil_DC = pd.DataFrame({'Days since 11/23':future_days_gr, 'Degree = 4':future_pred_2,'Degree = 5':future_pred_3})
