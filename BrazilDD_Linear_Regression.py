...
plt.plot(c, y_pred, 'r--')
plt.title('Brazil Daily Deaths: Linear Regression')
plt.xlabel('Days')
plt.ylabel('Deaths')
#plt.show()
...
LRfit_Brazil_DD = pd.DataFrame({'Dates': c, 'Deaths Prediction':y_pred, 'Deaths':y1test})
LR_November_DD = LRfit_Brazil_DD.to_excel("BrazilLRNovemberDeaths.xlsx", sheet_name='NovemberDailyDeathsPrediction')
...
LR_BrazilDD_pred = pd.DataFrame({'Days since 11/23':future_days , 'Deaths Prediction':future_pred})
LR_December_DD = LR_BrazilDD_pred.to_excel("BrazilLRDecemberDeaths.xlsx", sheet_name='DecemberDailyDeathsPrediction')

# Evaluation Table
LR_Brazil_Evaluation_DD = pd.DataFrame({'Model':lin_eq,'RMSE':[rmse], 'RMSLE':[rmsle], 'R2': [R2]})
...
print(LRfit_Brazil_DD)
print(LR_BrazilDD_pred)