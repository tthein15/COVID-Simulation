...
ytest = ytest.tolist()
SVRfit_India_DC = pd.DataFrame({'Dates': c, 'Cases Prediction':y_pred, 'Cases':ytest})
SVR_November_DC = SVRfit_India_DC.to_excel("IndiaSupportVectorRegressionNovemberCases.xlsx", sheet_name='November Daily Cases Prediction')
...
plt.title('Daily Cases In India: SVR')
...
# Excel File    
SVR_IndiaDC_pred = pd.DataFrame({'Days since 11/23':future_days , 'Cases Prediction':future_pred})
SVR_December_DC = SVR_IndiaDC_pred.to_excel("IndiaSupportVectorRegressionDecemberCases.xlsx", sheet_name='December Daily Cases Prediction')
